...
from src.pipelines.hele_pipeline_pose2vid import Pose2VideoPipeline, Pose2VideoPipeline_Stream
...
def parse_args():
    ...
    parser.add_argument("--use_taesd", action='store_true', help='Use Tiny AutoEncoder (TAESD) for faster decoding.')
    parser.add_argument("--stream_gen", type=bool, default=True, help='use streaming generation strategy to reduce VRAM usage.')
    parser.add_argument("--save_concat", action="store_true", help="Also save 4-row concat debug video (uses much more RAM on long runs).")
...
def main(args):
    ...
    if args.use_taesd:
        ...
        print("Using TAESD (Tiny AutoEncoder) for decoding")
    ...
    for ref_image_path in list(args.test_cases.keys()):
        for pose_video_path in args.test_cases[ref_image_path]:
            ...
            if args.save_concat:
                ...
                # Concat debug view: ref | driving face | original frame | generated output
