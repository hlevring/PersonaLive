    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.vae_tiny is not None:
            images = self.decode_latents_tiny(latents)
        ...
        with self.progress_bar(total=windows + temporal_adaptive_step - 1) as progress_bar:
            ...
            for i in range(windows + temporal_adaptive_step - 1):
                ...
                if i > temporal_adaptive_step - 2:
                    chunk = pred_original_sample[:,:,:temporal_window_size].to(dtype=self.dtype)
                    video_pile = self.decode_latents_tiny(chunk) if self.vae_tiny is not None else self.decode_latents(chunk)
